...
(* eval (NUM 4);
eval (DIV ((TIMES ((NUM 8), (MINUS ((PLUS ((NUM 4),(NUM 5))), (NUM 6))))), (NUM 8))); *)

(* 3 + 4 + 5 *)
(* val exps = [(DIV ((TIMES ((NUM 8), (MINUS ((PLUS ((NUM 4),(NUM 5))), (NUM 6))))), (NUM 8))), (NUM 4), (NUM 5)];
eval (F (exps, fn e => foldl (op +) 0 e));
eval (F ([], fn e => foldl (op +) 0 e)); *)
...
(* 
foldmap (fn e => e * e) [1,2,3,4];
foldmap (fn e => e + 1) [1,2,3,4];
foldmap (fn e => e) [1,2,3,4];
foldmap (fn e => e) []; *)

fun foldfilter (func: 'a -> bool): 'a list -> 'a list =  
    let 
        fun helper (a: 'a, L: 'a list) = if (func a) then a :: L else L
    in
        fn  (x: 'a list) => foldr helper [] x
    end ;

(* foldfilter (fn e => e = 3) [1,2,3,4];
foldfilter (fn e => e > 1) [1,2,3,4];
foldfilter (fn e => true) [1,2,3,4];
foldfilter (fn e => false) [1,2,3,4];
foldfilter (fn e => true) []; *)
...
(* count (fn e => e > 2) [~1,~2,~3,0,1,2,3,4];
count (fn e => e = 2) [~1,~2,~3,0,1,2,3,4];
count (fn e => e < 2) [~1,~2,~3,0,1,2,3,4];
count (fn e => e < 2) [];
count (fn e => e = []) [[],[],["a", "b"], [], ["c"]];
